...
import { CheckItem, ConfigDict, PatientField } from './sepseTypes';
import { 
  INITIAL_CONFIG, 
  INITIAL_CHECKLISTS, 
  NEWS_RAW_GRID_PARAMETERS, 
  SOFA_RAW_GRID_PARAMETERS 
} from './sepseDefaultData';
import { SepseConfigModal } from './SepseConfigModal';

import { VasopressorCalculator } from './VasopressorCalculator';
import { AutoResizeTextarea } from './AutoResizeTextarea';

export const EditableFicha = ({ localConfig, handleFieldChange, localChecklists, setLocalChecklists, localLogos, setLocalPatientFields, localPatientFields }: any) => {
  ...
  const T_bold = (key: string, _def: string, extraClasses: string = '') => (
    <AutoResizeTextarea
      ...
      className={`bg-transparent border-b border-dashed border-transparent hover:border-slate-350 focus:border-slate-500 focus:bg-white transition-all w-full outline-none resize-none block overflow-hidden ${extraClasses}`}
      rows={1}
    />
  );
  ...
  useEffect(() => {
    if ((localChecklists?.pacote || [])) {
      setCheckedPacote(prev => {
        const arr = [...prev];
        while (arr.length < (localChecklists?.pacote || []).length) arr.push(false);
        return arr.slice(0, (localChecklists?.pacote || []).length);
      });
    }
  }, [(localChecklists?.pacote || [])]);

  useEffect(() => {
    if ((localChecklists?.cuidados || [])) {
      setCheckedCuidados(prev => {
        const arr = [...prev];
        while (arr.length < (localChecklists?.cuidados || []).length) arr.push(false);
        return arr.slice(0, (localChecklists?.cuidados || []).length);
      });
    }
  }, [(localChecklists?.cuidados || [])]);
  ...
  const handleResetScores = () => {
    if (confirm('Deseja realmente limpar todas as marcações de triagem estimadas?')) {
      ...
      setCheckedPacote(new Array((localChecklists?.pacote || []).length).fill(false));
      setCheckedCuidados(new Array((localChecklists?.cuidados || []).length).fill(false));
  ...
  const themeColor = localConfig['_cor'] || '#185fa5';
      ...
      <div id="printable-ficha-content" className="max-w-[860px] mx-auto px-4 pt-6 space-y-5">
        ...
        <div id="pop-header" className="bg-white rounded-2xl border-2 border-emerald-800 overflow-hidden shadow-md font-sans">
          ...
          <div className="grid grid-cols-3 divide-x divide-emerald-800 border-t border-emerald-800 bg-emerald-600 text-white text-center text-[10px] font-black uppercase">
            <div className="py-2">{T_bold('pop.codigo', 'Código: POP-HMM-001')}</div>
            <div className="py-2">{T_bold('pop.versao', 'VERSÃO: 01')}</div>
            <div className="py-2">{T_bold('pop.pagina', 'PÁGINA 1')}</div>
          </div>
          ...
          <div className="grid grid-cols-3 divide-x divide-emerald-100 bg-white border-t border-emerald-800 text-[10px] text-center uppercase font-bold text-slate-500">
            <div className="py-2">
              ...
              <span className="text-slate-800 font-extrabold">{T_bold('pop.data_elab', '25/05/2026')}</span>
            </div>
            <div className="py-2">
              <span className="text-[8px] block text-slate-400">Data de Aprovação</span>
              <span className="text-slate-800 font-extrabold">{T_bold('pop.data_aprov', '26/05/2026')}</span>
            </div>
            <div className="py-2">
              <span className="text-[8px] block text-slate-400">Próxima Revisão</span>
              <span className="text-slate-800 font-extrabold">{T_bold('pop.data_rev', '26/05/2027')}</span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3 shadow-sm select-none">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-black text-slate-900 tracking-tight">{T_bold('header.titulo', 'Sepse')}</h1>
              <span className="bg-blue-100 text-blue-800 text-[9px] font-black px-2.5 py-0.5 rounded-full tracking-wider">{T_bold('header.badge', 'SSC 2026')}</span>
            </div>
            <p className="text-[11px] text-slate-500 font-bold mt-1">{T_bold('header.sub', 'Baseado nas diretrizes Surviving Sepsis Campaign (SSC) 2026')}</p>
          </div>
          <div className="md:text-right">
            <span className="inline-block text-[9px] font-black tracking-widest text-[#185fa5] bg-blue-50/50 border border-blue-200/50 rounded-full px-3 py-1 mb-1 bg-opacity-70">
              {T_bold('header.hospital', 'Hospital Municipal de Morrinhos')}
            </span>
            <p className="text-xs font-black text-slate-700">{T_bold('header.autor', 'Dr. Ivaldo Inácio Silva Júnior')}</p>
            <p className="text-[9px] text-slate-400 font-extrabold uppercase tracking-wide mt-0.5">{T_bold('header.cargo', 'Diretor Técnico do Hospital Municipal de Morrinhos')}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4 shadow-sm no-print">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {localPatientFields.filter(f => f.visible).map(field => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 select-none">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 flex items-center gap-4 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-400 font-bold tracking-tight mt-1">
                {T_bold('scores.news.detail', 'Pontuação de triagem para sepse · NOTÍCIAS ≥ 4 = alta acurácia')}
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 flex items-center gap-4 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-400 font-bold tracking-tight mt-1">
                {T_bold('scores.sofa.detail', 'Disfunção orgânica · SOFA ≥ 2 + infecção = Sepse')}
              </p>
            </div>
          </div>
          ...
          if (sectionId === 's3') {
            return (
              <div key="s3" className="prot-section space-y-3 pt-2">
                <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{T_bold('s3.label', '')}</div>
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-slate-900">{T_bold('s3.titulo', '')}</h4>
                        <p className="text-[10px] text-slate-400 font-bold mt-0.5">{T_bold('s3.sub', 'Categorização de condutas imediatas baseadas na suspeita')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">{T_bold('s3.range', 'SSC 2026')}</span>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-2">
                        ...
                        <div className={`p-4 rounded-xl border transition-all flex flex-col justify-between ${derivedRiskCategory === 'IMPROVAVEL' ? 'bg-emerald-50 border-emerald-500 scale-[1.01] shadow' : 'bg-slate-50 border-slate-200/50 opacity-60'}`}>
                          <div>
                            ...
                            <h5 className="text-xs font-black text-slate-800 mt-2">{T_bold('s3.c1.titulo', 'SEPSE IMPROVÁVEL')}</h5>
                            <p className="text-[10px] text-slate-500 whitespace-pre-wrap leading-relaxed mt-1.5">{T_bold('s3.c1.desc', '')}</p>
                          </div>
                        </div>
                        ...
                        <div className={`p-4 rounded-xl border transition-all flex flex-col justify-between ${derivedRiskCategory === 'POSSIVEL' ? 'bg-amber-50 border-amber-500 scale-[1.01] shadow' : 'bg-slate-50 border-slate-200/50 opacity-60'}`}>
                          <div>
                            ...
                            <h5 className="text-xs font-black text-slate-800 mt-2">{T_bold('s3.c2.titulo', 'SEPSE POSSÍVEL')}</h5>
                            <p className="text-[10px] text-slate-500 whitespace-pre-wrap leading-relaxed mt-1.5">{T_bold('s3.c2.desc', '')}</p>
                          </div>
                        </div>
                        ...
                        <div className={`p-4 rounded-xl border transition-all flex flex-col justify-between ${derivedRiskCategory === 'PROVAVEL' ? 'bg-orange-50 border-orange-500 scale-[1.01] shadow' : 'bg-slate-50 border-slate-200/50 opacity-60'}`}>
                          <div>
                            ...
                            <h5 className="text-xs font-black text-slate-800 mt-2">{T_bold('s3.c3.titulo', 'SEPSE PROVÁVEL')}</h5>
                            <p className="text-[10px] text-slate-500 whitespace-pre-wrap leading-relaxed mt-1.5">{T_bold('s3.c3.desc', '')}</p>
                          </div>
                        </div>
                        ...
                        <div className={`p-4 rounded-xl border transition-all flex flex-col justify-between ${derivedRiskCategory === 'DEFINIDA' ? 'bg-rose-50 border-rose-500 scale-[1.01] shadow' : 'bg-slate-50 border-slate-200/50 opacity-60'}`}>
                          <div>
                            ...
                            <h5 className="text-xs font-black text-slate-850 mt-2">{T_bold('s3.c4.titulo', 'SEPSE DEFINIDA')}</h5>
                            <p className="text-[10px] text-slate-500 whitespace-pre-wrap leading-relaxed mt-1.5">{T_bold('s3.c4.desc', '')}</p>
                          </div>
                        </div>
          ...
          if (sectionId === 's1') {
            return (
              <div key="s1" className="prot-section space-y-3 pt-2">
                <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{T_bold('s1.label', '')}</div>
                      ...
                      <div key={param.id} className={`bg-white rounded-xl border transition-all overflow-hidden ${isExpanded ? 'border-blue-500 shadow' : 'border-slate-200 hover:border-slate-300'}`}>
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h4 className="text-xs font-black text-slate-850">{localConfig[param.titleKey]}</h4>
                              <p className="text-[9px] text-slate-400 font-bold mt-0.5">{localConfig[param.subKey]}</p>
                            </div>
                          </div>
                        ...
                        {isExpanded && (
                          <div className="p-4 border-t border-slate-100 bg-white space-y-4">
                            ...
                            <div className="p-3.5 bg-blue-50/50 border border-blue-100 rounded-xl text-blue-900 text-[10px] space-y-1">
                              ...
                              <p className="font-bold leading-relaxed">{localConfig[param.howToKey]}</p>
                            </div>

                            {/* Attention Box in orange */}
                            {param.attentionKey && localConfig[param.attentionKey] && (
                              <div className="p-3.5 bg-amber-50/40 border border-amber-200/50 rounded-xl text-amber-900 text-[10px] space-y-1">
                                ...
                                <p className="font-bold leading-relaxed">{localConfig[param.attentionKey]}</p>
                              </div>
                            )}
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[11px] font-bold">{localConfig[opt.labelKey]}</span>
                                      </div>
          ...
          if (sectionId === 's2') {
            return (
              <div key="s2" className="prot-section space-y-3 pt-2">
                <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{T_bold('s2.label', '')}</div>
                <div className="space-y-2">
                      ...
                      <div key={param.id} className={`bg-white rounded-xl border transition-all overflow-hidden ${isExpanded ? 'border-red-500 shadow' : 'border-slate-200 hover:border-slate-300'}`}>
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <h4 className="text-xs font-black text-slate-800">{localConfig[param.titleKey]}</h4>
                              <p className="text-[9px] text-slate-400 font-bold mt-0.5">{localConfig[param.subKey]}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {isExpanded && (
                          <div className="p-4 border-t border-slate-100 bg-white space-y-4">
                            ...
                            <div className="p-3.5 bg-blue-50/50 border border-blue-100 rounded-xl text-blue-900 text-[10px] space-y-1">
                              <span className="block text-[8px] font-black tracking-widest text-[#185fa5] uppercase">COMO AVALIAR:</span>
                              {param.id === 'cv' && typeof localConfig[param.howToKey] === 'string' && localConfig[param.howToKey].includes('🔴 NORADRENALINA') ? (() => {
                                const text = localConfig[param.howToKey];
                                ...
                                return <p className="font-bold leading-relaxed whitespace-pre-wrap">{localConfig[param.howToKey]}</p>;
                              })() : (
                                <p className="font-bold leading-relaxed whitespace-pre-wrap">{localConfig[param.howToKey]}</p>
                              )}
                            </div>
                                  ...
                                  return (
                                    ...
                                    >
                                      <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[11px] font-bold whitespace-pre-wrap">{localConfig[opt.labelKey]}</span>
                                      </div>
                                      {isActive && <Check className="w-4 h-4 text-emerald-400 flex-shrink-0" />}
                                    </button>
                                  );
          ...
          if (sectionId === 's4') {
            return (
              <div key="s4" className="prot-section space-y-3 pt-2">
                <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{T_bold('s4.label', '')}</div>
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-slate-900">{T_bold('s4.titulo', '')}</h4>
                        <p className="text-[10px] text-slate-400 font-bold mt-0.5">{T_bold('s4.sub', '')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">{T_bold('s4.range', '')}</span>
                  ...
                  {(!expandedItems['pacote-checklist']) && (
                    <div className="p-4.5 border-t border-slate-100 bg-white space-y-4">
                      ...
                      <div className="p-3.5 bg-rose-50/50 border border-thin rounded-xl text-rose-900 text-[11px] font-bold no-print">
                        ...
                        <p className="leading-relaxed mt-0.5 whitespace-pre-wrap">{T_bold('s4.blk1.texto', '')}</p>
                      </div>

                      {/* Checklist nodes */}
                      <div className="space-y-1.5 font-sans">
                        {(localChecklists?.pacote || []).map((item, idx) => {
          ...
          if (sectionId === 's5') {
            return (
              <div key="s5" className="prot-section space-y-3 pt-2">
                <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{T_bold('s5.label', '')}</div>
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-slate-900">{T_bold('s5.titulo', '')}</h4>
                        <p className="text-[10px] text-slate-400 font-bold mt-0.5">{T_bold('s5.sub', '')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">{T_bold('s5.range', '')}</span>
                    ...
                    <div className="p-4.5 border-t border-slate-100 bg-white space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-1 no-print">
                        <div className="p-3.5 bg-blue-50/50 border border-blue-100 rounded-xl text-blue-900 text-[10px] space-y-1">
                          ...
                          <p className="font-bold leading-relaxed whitespace-pre-wrap">{T_bold('s5.blk1.texto', '')}</p>
                        </div>
                        <div className="p-3.5 bg-emerald-50/50 border border-emerald-100 rounded-xl text-emerald-900 text-[10px] space-y-1">
                          ...
                          <p className="font-bold leading-relaxed whitespace-pre-wrap">{T_bold('s5.blk2.texto', '')}</p>
                        </div>
                      </div>

                      {/* Noradrenalina prescribing details */}
                      <div className="bg-slate-50/80 border p-4 rounded-xl space-y-2">
                        <span className="block text-[9px] font-black text-[#185fa5] uppercase tracking-wider">{T_bold('s5.nora.label', '')}</span>
                        <p className="text-[11px] font-bold text-slate-600">{T_bold('s5.nora.indicacao', '')}</p>
                        <p className="text-[11px] font-bold text-slate-600 border-b pb-2">{T_bold('s5.nora.meta', '')}</p>
                        <div className="pt-1.5">
                          <span className="block text-[8px] font-extrabold text-slate-400 tracking-wider uppercase mb-1">{T_bold('s5.nora.rx.label', 'Prescrição Prática:')}</span>
                      ...
                      <div className="bg-slate-50/80 border p-4 rounded-xl space-y-2">
                        <span className="block text-[9px] font-black text-[#185fa5] uppercase tracking-wider">{T_bold('s5.vaso.label', '')}</span>
                        <p className="text-[11px] font-bold text-slate-600">{T_bold('s5.vaso.indicacao', '')}</p>
                        <div className="pt-1.5">
                          <span className="block text-[8px] font-extrabold text-slate-400 tracking-wider uppercase mb-1">{T_bold('s5.vaso.rx.label', 'Prescrição Prática:')}</span>
                          <div className="p-2.5 bg-white border rounded font-mono text-[10px] leading-relaxed text-slate-700 whitespace-pre-wrap overflow-x-auto shadow-inner">
                            {T_bold('s5.vaso.rx.texto', '')}
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Corticoides details */}
                      <div className="bg-slate-50/80 border p-4 rounded-xl space-y-2">
                        <span className="block text-[9px] font-black text-[#185fa5] uppercase tracking-wider">{T_bold('s5.corti.label', '')}</span>
                        <p className="text-[11px] font-bold text-slate-600">{T_bold('s5.corti.indicacao', '')}</p>
                        <div className="pt-1.5">
                          <span className="block text-[8px] font-extrabold text-slate-400 tracking-wider uppercase mb-1">{T_bold('s5.corti.rx.label', 'Informações Práticas:')}</span>
          ...
          if (sectionId === 's6') {
            return (
              <div key="s6" className="prot-section space-y-3 pt-2">
                <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{T_bold('s6.label', '')}</div>
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-slate-900">{T_bold('s6.titulo', '')}</h4>
                        <p className="text-[10px] text-slate-400 font-bold mt-0.5">{T_bold('s6.sub', '')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">{T_bold('s6.range', '')}</span>
                  ...
                  {(!expandedItems['antibioticos']) && (
                    <div className="p-4 border-t border-slate-100 bg-white space-y-4">
                      ...
                      <div className="p-3.5 bg-rose-50/50 border border-thin rounded-xl text-rose-900 text-[11px] font-bold no-print">
                        ...
                        <p className="leading-relaxed mt-0.5 whitespace-pre-wrap">{T_bold('s6.blk1.texto', '')}</p>
                      </div>
                          ...
                          <tbody className="divide-y divide-slate-200 text-slate-700 leading-relaxed font-medium align-top">
                            ...
                            <tr className="hover:bg-slate-50 transition-colors">
                              ...
                              >
                               ...
                               <td className="p-3 bg-slate-50/50 font-black text-slate-900 border-r border-slate-200">{T_bold('s6.t.pulm.foco', '')}</td>
                            ...
                            <tr className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="p-3 bg-slate-50/50 font-black text-slate-900 border-r border-slate-200">{T_bold('s6.t.abd.foco', '')}</td>
                              <td className="p-3 border-r border-slate-200"><span className="font-bold text-slate-900">Piperacilina-Tazobactam 4,5g</span></td>
                              <td className="p-3 border-r border-slate-200">100mL SF 0,9%</td>
                              <td className="p-3 border-r border-slate-200 font-extrabold text-blue-700">EV</td>
                              <td className="p-3 border-r border-slate-200">6/6H</td>
                            ...
                            <tr className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="p-3 bg-slate-50/50 font-black text-slate-900 border-r border-slate-200">{T_bold('s6.t.urin.foco', '')}</td>
                              <td className="p-3 border-r border-slate-200"><span className="font-bold text-slate-900">Piperacilina-Tazobactam 4,5g</span></td>
                              <td className="p-3 border-r border-slate-200">100mL SF 0,9%</td>
                              <td className="p-3 border-r border-slate-200 font-extrabold text-blue-700">EV</td>
                              <td className="p-3 border-r border-slate-200">6/6H</td>
                            ...
                            <tr className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="p-3 bg-slate-50/50 font-black text-slate-900 border-r border-slate-200">{T_bold('s6.t.mening.foco', '')}</td>
                            ...
                            <tr className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="p-3 bg-slate-50/50 font-black text-slate-900 border-r border-slate-200">{T_bold('s6.t.cut.foco', '')}</td>
                              <td className="p-3 border-r border-slate-200"><span className="font-bold text-slate-900">Ceftriaxona 2g</span></td>
                              <td className="p-3 border-r border-slate-200">100mL SF 0,9%</td>
                              <td className="p-3 border-r border-slate-200 font-extrabold text-blue-700">EV</td>
                            ...
                            <tr className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="p-3 bg-slate-50/50 font-black text-slate-900 border-r border-slate-200">{T_bold('s6.t.sangue.foco', '')}</td>
                              <td className="p-3 border-r border-slate-200"><span className="font-bold text-slate-900">Piperacilina-Tazobactam 4,5g</span></td>
                              <td className="p-3 border-r border-slate-200">100mL SF 0,9%</td>
                              <td className="p-3 border-r border-slate-200 font-extrabold text-blue-700">EV</td>
                              <td className="p-3 border-r border-slate-200">6/6H</td>
                            ...
                            <tr className="hover:bg-slate-50 transition-colors">
                              ...
                              <td className="p-3 bg-slate-50/50 font-black text-slate-900 border-r border-slate-200">{T_bold('s6.t.sfoco.foco', '')}</td>
                              <td className="p-3 border-r border-slate-200"><span className="font-bold text-slate-900">Piperacilina-Tazobactam 4,5g</span></td>
                              <td className="p-3 border-r border-slate-200">100mL SF 0,9%</td>
                              <td className="p-3 border-r border-slate-200 font-extrabold text-blue-700">EV</td>
                              <td className="p-3 border-r border-slate-200">6/6H</td>
          ...
          if (sectionId === 's7') {
            return (
              <div key="s7" className="prot-section space-y-3 pt-2">
                <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{T_bold('s7.label', '')}</div>
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-750 font-bold text-xs flex items-center justify-center">☰</div>
                      <div>
                        <h4 className="text-sm font-black text-slate-900">{T_bold('s7.titulo', '')}</h4>
                        <p className="text-[10px] text-slate-400 font-bold mt-0.5">{T_bold('s7.sub', 'VM · TEV · LAMG · Glicemia · Monitorização')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest">{T_bold('s7.range', 'Internação')}</span>
                  ...
                  {(!expandedItems['cuidados-checklist']) && (
                    <div className="p-4.5 border-t border-slate-100 bg-white space-y-4">
                      ...
                      <div className="space-y-1.5 font-sans">
                        {(localChecklists?.cuidados || []).map((item, idx) => {
        ...
        <div className="pt-8 border-t border-slate-200/80 flex flex-col md:flex-row md:items-center md:justify-between gap-4 select-none">
          <div className="text-[10px] text-slate-400 font-bold max-w-sm leading-relaxed">
            {T_bold('footer.fonte', 'Baseado nas diretrizes Surviving Sepsis Campaign (SSC) 2026')}
          </div>
          <div className="text-left md:text-right">
            <h5 className="text-xs font-black text-slate-700">{T_bold('footer.autor', 'Dr. Ivaldo Inácio Silva Júnior')}</h5>
            <p className="text-[10px] text-slate-400 font-extrabold uppercase mt-0.5 tracking-wide">{T_bold('footer.cargo', 'Diretor Técnico do Hospital Municipal de Morrinhos')}</p>
          </div>
        </div>
